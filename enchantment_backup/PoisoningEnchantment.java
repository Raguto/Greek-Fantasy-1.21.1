...
public class PoisoningEnchantment extends Enchantment {
    ...
    private static final ResourceLocation SNAKESKIN = ResourceLocation.fromNamespaceAndPath(GreekFantasy.MODID, "armor/snakeskin");
    ...
    public void doPostHurt(LivingEntity user, Entity attacker, int level) {
        ...
        if (shouldHit(level, user.getRandom())) {
            if (attacker instanceof LivingEntity) {
                ...
                ((LivingEntity) attacker).addEffect(new MobEffectInstance(MobEffects.POISON.getDelegate(), duration, level));
            }

            if (enchants != null) {
                enchants.getValue().hurtAndBreak(2, user, enchants.getKey());
            }
        }
    }
    ...
    public boolean canApplyAtEnchantingTable(ItemStack stack) {
        return stack.is(TagKey.create(BuiltInRegistries.ITEM.key(), SNAKESKIN)) && super.canApplyAtEnchantingTable(stack);
    }
}
*/
*/
