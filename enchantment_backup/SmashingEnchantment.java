...
public class SmashingEnchantment extends Enchantment {
    ...
    private static final ResourceLocation CLUB = ResourceLocation.fromNamespaceAndPath(GreekFantasy.MODID, "tools/club");
    ...
    private static void useSmashAttack(final LivingEntity user, final Entity target, final int level) {
        ...
        if (target.onGround() && !isExemptFromSmashAttack(target)) {
            ...
            target.hurt(target.damageSources().mobAttack(user), 0.25F);
            ...
            if (target instanceof LivingEntity) {
                ...
                if (GreekFantasy.CONFIG.SMASHING_NERF.get()) {
                    entity.addEffect(new MobEffectInstance(MobEffects.MOVEMENT_SLOWDOWN.getDelegate(), duration, 0));
                    entity.addEffect(new MobEffectInstance(MobEffects.WEAKNESS.getDelegate(), duration, 0));
                } else {
    ...
    /*public static void apply(LivingEntity user, Entity target, int level, float attackStrength) {
        ...
        if (!item.is(TagKey.create(BuiltInRegistries.ITEM.key(), CLUB))) {
            return;
        }
    ...
    public boolean canApplyAtEnchantingTable(ItemStack stack) {
        return stack.is(TagKey.create(BuiltInRegistries.ITEM.key(), CLUB)) && super.canApplyAtEnchantingTable(stack);
    }
