...
public class Whirl extends WaterAnimal {
    ...
    protected static final TagKey<Item> TRIGGER = TagKey.create(BuiltInRegistries.ITEM.key(), ResourceLocation.fromNamespaceAndPath(GreekFantasy.MODID, "charybdis_trigger"));
    ...
    public void defineSynchedData(SynchedEntityData.Builder builder) {
        ...
        builder.define(ATTRACT_MOBS, Boolean.FALSE);
    }
    ...
    public void aiStep() {
        ...
        // remove nearby items
        final double maxItemY = this.getY() + this.getBbHeight() + 0.75D;
        final List<ItemEntity> itemEntityList = this.level().getEntities(EntityType.ITEM, this.getBoundingBox().inflate(1.25D, 1.0F, 1.25D), e -> e.getY() < maxItemY);
        for (final ItemEntity e : itemEntityList) {
            // check for trigger items
            if (this.level() instanceof ServerLevel && !e.getItem().isEmpty()) {
                Item item = e.getItem().getItem();
                if (e.getItem().is(TRIGGER) || item == Items.HEART_OF_THE_SEA) {
            ...
            if (e.isInWaterOrBubble() && !e.hasPickUpDelay()) {
                e.discard();
            }
    ...
    public void baseTick() {
        ...
        super.baseTick();
        // Restore air supply to prevent drowning - whirls can breathe underwater
        if (this.isInWater()) {
            this.setAirSupply(i);
        }
    }
    ...
    protected void actuallyHurt(final DamageSource source, final float amountIn) {
        ...
        if (!source.is(DamageTypeTags.BYPASSES_ENCHANTMENTS) && getAttractMobs()) {
            amount *= 0.25F;
        }
    ...
    private static class SwirlGoal extends greekfantasy.entity.ai.SwirlGoal {
        ...
        protected void onCollideWith(Entity e) {
            ...
            if (whirl.getAttractMobs() && e instanceof LivingEntity) {
                e.hurt(e.damageSources().mobAttack(entity), 1.0F);
            }
        }
    }
}
