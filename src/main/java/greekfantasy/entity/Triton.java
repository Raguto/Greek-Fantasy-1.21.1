...
public class Triton extends PathfinderMob implements RangedAttackMob, NeutralMob, TradingMob {
    ...
    protected static final TagKey<Item> TRITON_TRADE = ItemTags.create(ResourceLocation.fromNamespaceAndPath(GreekFantasy.MODID, "triton_trade"));
    protected static final ResourceLocation TRADE_LOOT_TABLE = ResourceLocation.fromNamespaceAndPath(GreekFantasy.MODID, "gameplay/triton_trade");
    ...
    protected void registerGoals() {
        ...
        if(GreekFantasy.CONFIG.TRITON_SEEK_OCEAN_VILLAGE.get()) {
            this.goalSelector.addGoal(6, new MoveToStructureGoal(this, 1.0D, 6, 4, 10, ResourceLocation.fromNamespaceAndPath(GreekFantasy.MODID, "ocean_village"), BehaviorUtils::getRandomSwimmablePos));
        }
    ...
    public void baseTick() {
        ...
        super.baseTick();
        // Restore air supply to prevent drowning - tritons can breathe underwater
        if (this.isInWater()) {
            this.setAirSupply(i);
        }
    }
    ...
    // TODO 1.21.1: getDimensions(Pose) is now FINAL in Entity - cannot override
    // Original implementation returned swimmingDimensions when pose == Pose.SWIMMING
    ...
    protected Component getTypeName() {
        if (null == description) {
            ...
            this.description = Component.translatable(descriptionId);
        }
        return description;
    }
    ...
    public Optional<Guardian> trySpawnGuardian(ServerLevel level) {
        ...
        for (int i = 0; i < 10; ++i) {
            ...
            if (level.getBlockState(pos).getFluidState().is(FluidTags.WATER)) {
                // 1.21: EntityType.create signature changed - no longer takes Consumer, BlockPos directly
                Guardian guardian = EntityType.GUARDIAN.create(level);
                if (guardian != null) {
                    guardian.moveTo(pos.getX() + 0.5, pos.getY(), pos.getZ() + 0.5, 0, 0);
                    if (guardian.checkSpawnRules(level, MobSpawnType.MOB_SUMMONED) && guardian.checkSpawnObstruction(level)) {
                        ...
                        // TODO 1.21: Capability system removed - needs NeoForge attachment system
        ...
        if (super.hurt(source, amount) && source.getEntity() instanceof Enemy) {
            if (this.level() instanceof ServerLevel serverLevel) {
                ...
                if (wantsToSpawnGuardian.size() >= 2) {
                    ...
                    // TODO 1.21: Capability system removed - needs NeoForge attachment system
                    // For now, just check for any guardians nearby
                    List<Guardian> guardians = level().getEntitiesOfClass(Guardian.class, aabb);
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor worldIn, DifficultyInstance difficultyIn, MobSpawnType mobType,
        ...
        boolean slim = getRandom().nextBoolean();
        this.setSlim(slim);
        populateDefaultEquipmentSlots(getRandom(), difficultyIn);
        return result;
    }

    // TODO 1.21.1: canBreatheUnderwater() is now FINAL in Entity - cannot override
    // This entity extends PathfinderMob which doesn't breathe underwater by default
    // Consider using a different approach such as overriding decreaseAirSupply or using MobType.WATER
    // @Override
