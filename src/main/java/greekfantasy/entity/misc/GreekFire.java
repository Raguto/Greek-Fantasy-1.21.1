...
public class GreekFire extends ThrowableItemProjectile {
    ...
    protected Explosion causeExplosion(final Vec3 vec) {
        ...
        Explosion exp = this.level().explode(this.getOwner(), damageSources().onFire(), null, vec.x, vec.y, vec.z, size, false, Level.ExplosionInteraction.NONE);
            ...
            for(float y = -size2; y < size2; y++) {
                for(float z = -size2; z < size2; z++) {
                    ...
                    if (level().random.nextInt(3) > 0) {
                        ...
                        if ((state.is(BlockTags.REPLACEABLE) && level().getBlockState(pos.below()).isSolidRender(level(), pos.below()))) {
