...
public class Pegasus extends AbstractHorse implements FlyingAnimal, HasHorseVariant {
    ...
    public void aiStep() {
        ...
        if (this.position().y > this.level().getHeight() + 16) {
            this.hurt(damageSources().outOfBorder(), 2.0F);
        }
    }
    ...
    protected float generateRandomMaxHealth(RandomSource random) {
        return super.generateMaxHealth(random::nextInt) + 10.0F;
    }

    protected double generateRandomJumpStrength(RandomSource random) {
        return super.generateJumpStrength(random::nextDouble) + 0.20F;
    }

    protected double generateRandomSpeed(RandomSource random) {
        return super.generateSpeed(random::nextDouble);
    }
    ...
    class AvoidPlayersGoal extends AvoidEntityGoal<Player> {
        ...
        public boolean canUse() {
            // TODO: In 1.21+, pathNav is final and cannot be reassigned.
            // The original code tried to update pathNav to handle navigation switching (flying vs ground).
            // This may need a different approach - perhaps a custom goal implementation.
            // For now, rely on the navigation set at construction time.
