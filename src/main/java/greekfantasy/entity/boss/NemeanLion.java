...
public class NemeanLion extends Monster implements HasCustomCooldown {
    ...
    protected void registerGoals() {
        ...
        if(GreekFantasy.CONFIG.NEMEAN_LION_SEEK_DEN.get()) {
            this.goalSelector.addGoal(4, new MoveToStructureGoal(this, 1.0D, 3, 8, 4, ResourceLocation.fromNamespaceAndPath(GreekFantasy.MODID, "lion_den"), DefaultRandomPos::getPos));
        }
        ...
        this.targetSelector.addGoal(3, new NearestAttackableTargetGoal<>(this, Animal.class, 10, false, false, e -> e.canChangeDimensions(e.level(), e.level()) && !e.getType().is(BOSSES) && !e.isInWater() && !NemeanLion.this.isVehicle()));
    ...
    public void aiStep() {
        ...
        if (isVehicle() && getPassengers().get(0) instanceof Player) {
            ...
            if (this.hurtTime == 0 && !level().isClientSide()) {
                this.hurt(damageSources().playerAttack(player), 1.0F + random.nextFloat());
                // remove regen
    ...
    public boolean isInvulnerableTo(final DamageSource source) {
        return source ==  damageSources().inWall() || source.is(DamageTypeTags.WITHER_IMMUNE_TO)
                || source == damageSources().cactus() || source == damageSources().sweetBerryBush()
    ...
    class NemeanLionAttackGoal extends CooldownMeleeAttackGoal<NemeanLion> {
        ...
        // Note: getAttackReachSqr removed in 1.21 - attack reach is now handled differently
    }
