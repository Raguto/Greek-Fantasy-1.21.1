...
public class Python extends Monster implements RangedAttackMob {
    ...
    // TODO 1.21: MobType removed - using entity tags instead
    private static final TagKey<EntityType<?>> UNDEAD = TagKey.create(BuiltInRegistries.ENTITY_TYPE.key(), ResourceLocation.fromNamespaceAndPath("minecraft", "undead"));
    private static final TagKey<EntityType<?>> ARTHROPOD = TagKey.create(BuiltInRegistries.ENTITY_TYPE.key(), ResourceLocation.fromNamespaceAndPath("minecraft", "arthropod"));
    private static final TagKey<EntityType<?>> AQUATIC = TagKey.create(BuiltInRegistries.ENTITY_TYPE.key(), ResourceLocation.fromNamespaceAndPath("minecraft", "aquatic"));

    private static final Predicate<LivingEntity> TARGET_SELECTOR = (e) -> {
        return e.isAlive() && !e.getType().is(BOSSES) && e.canChangeDimensions(e.level(), e.level())
                && !e.getType().is(UNDEAD) && !e.getType().is(ARTHROPOD) && !e.getType().is(AQUATIC);
    };
    ...
    public boolean isInvulnerableTo(final DamageSource source) {
        return source == damageSources().inWall() || source.is(DamageTypeTags.WITHER_IMMUNE_TO) || super.isInvulnerableTo(source);
    }
