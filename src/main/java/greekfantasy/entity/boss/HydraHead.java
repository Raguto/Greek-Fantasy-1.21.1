...
public class HydraHead extends Monster {
    ...
    public void defineSynchedData(SynchedEntityData.Builder builder) {
        ...
        builder.define(PART_ID, (byte) 0);
        builder.define(STATE, NORMAL);
    }
    ...
    public boolean isInvulnerableTo(final DamageSource source) {
        return ((isSevered() || isGrowing()) && !source.is(DamageTypeTags.IS_FIRE))
                || isCharred() || source == damageSources().inWall() || source.is(DamageTypeTags.WITHER_IMMUNE_TO)
    ...
    protected InteractionResult mobInteract(final Player player, final InteractionHand hand) {
        ...
        if (!itemstack.isEmpty() && itemstack.is(Items.FLINT_AND_STEEL)) {
            ...
            if (!this.level().isClientSide()) {
                this.igniteForSeconds(4 + random.nextInt(3));
    ...
    // Original implementation returned severedSize when not normal
    ...
    // Note: getStandingEyeHeight no longer overrideable for dynamic sizes in 1.21
    // Eye height is handled via EntityType builder
