...
public class Geryon extends Monster implements HasCustomCooldown {
    ...
    private boolean isExemptFromSmashAttack(final Entity entity) {
        return !entity.canChangeDimensions(entity.level(), entity.level()) || entity.getType().is(BOSSES)
    ...
    private void useSmashAttack(final Entity entity) {
        ...
        if (entity.onGround() && !isExemptFromSmashAttack(entity)) {
            ...
            entity.hurt(damageSources().mobAttack(this), (float) this.getAttribute(Attributes.ATTACK_DAMAGE).getValue());
