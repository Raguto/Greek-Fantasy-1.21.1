...
public class Cyclops extends Monster {
    ...
    protected void registerGoals() {
        ...
        if(GreekFantasy.CONFIG.CYCLOPS_SEEK_CAVE.get()) {
            this.goalSelector.addGoal(4, new MoveToStructureGoal(this, 1.0D, 4, 8, 4, ResourceLocation.fromNamespaceAndPath(GreekFantasy.MODID, "cyclops_cave"), DefaultRandomPos::getPos));
        }
    ...
    // Note: dropCustomDeathLoot signature changed in 1.21 (first param is now ServerLevel)
    // Removed empty override that just called super
    ...
    protected void populateDefaultEquipmentSlots(RandomSource random, DifficultyInstance difficulty) {
        ...
        if (random.nextBoolean()) {
            ...
            if (random.nextFloat() < 0.10F * difficulty.getSpecialMultiplier()) {
                greekfantasy.enchantment.EnchantmentUtils.enchant(level(), itemStack, net.minecraft.world.item.enchantment.Enchantments.KNOCKBACK, 1);
            }
