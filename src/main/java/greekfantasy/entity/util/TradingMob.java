...
public interface TradingMob {
    ...
    default List<ItemStack> getTradeResult(final PathfinderMob self, @Nullable final Player player, final ItemStack tradeItem) {
        net.minecraft.resources.ResourceKey<LootTable> lootTableKey = net.minecraft.resources.ResourceKey.create(
            net.minecraft.core.registries.Registries.LOOT_TABLE, this.getTradeLootTable());
        LootTable loottable = self.level().getServer().reloadableRegistries().getLootTable(lootTableKey);
        return loottable.getRandomItems(new LootParams.Builder((ServerLevel) self.level())
