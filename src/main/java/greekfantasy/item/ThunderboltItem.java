    ...
    public InteractionResultHolder<ItemStack> use(final Level level, final Player player, final InteractionHand hand) {
        ...
        if (!level.isClientSide()) {
            ...
            if (raytrace.getType() != HitResult.Type.MISS) {
                ...
                // TODO_ENCHANT: Fireflash enchantment disabled
                final int fireflashLevel = 0; // stack.getEnchantmentLevel(null);
                final boolean fireflash = false; // GreekFantasy.CONFIG.FIREFLASH_ENABLED.get() && fireflashLevel > 0 && (!GreekFantasy.isRGLoaded() || RGCompat.getInstance().canUseFireflash(player));
                // if enchanted with fireflash, cause an explosion
                if (fireflash) {
                    damageAmount = GreekFantasy.CONFIG.THUNDERBOLT_DURABILITY_ON_FIREFLASH.get();
                    final Level.ExplosionInteraction mode = GreekFantasy.CONFIG.FIREFLASH_DESTROYS_BLOCKS.get() ? Level.ExplosionInteraction.TNT : Level.ExplosionInteraction.NONE;
