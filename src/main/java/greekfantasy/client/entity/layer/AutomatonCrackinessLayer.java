...
public class AutomatonCrackinessLayer<T extends Automaton, M extends EntityModel<T>> extends RenderLayer<T, M> {

    private static final ResourceLocation[] CRACKINESS = {
            ResourceLocation.fromNamespaceAndPath(GreekFantasy.MODID, "textures/entity/automaton/automaton_crackiness_high.png"),
            ResourceLocation.fromNamespaceAndPath(GreekFantasy.MODID, "textures/entity/automaton/automaton_crackiness_medium.png"),
            ResourceLocation.fromNamespaceAndPath(GreekFantasy.MODID, "textures/entity/automaton/automaton_crackiness_low.png")
    };
    ...
    public void render(PoseStack poseStack, MultiBufferSource multiBufferSource, int packedLight, T entity, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
        ...
        if (crackiness >= 0 && crackiness < 3 && !entity.isInvisible()) {
            ...
            getParentModel().renderToBuffer(poseStack, vertexconsumer, packedLight, LivingEntityRenderer.getOverlayCoords(entity, 0.0F), 0x80FFFFFF);
        }
    }
}
