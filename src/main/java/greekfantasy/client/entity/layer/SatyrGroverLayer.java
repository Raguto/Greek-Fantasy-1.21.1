...
public class SatyrGroverLayer<T extends Satyr, M extends EntityModel<T>> extends RenderLayer<T, M> {
    ...
    private static final ResourceLocation GROVER_OVERLAY_TEXTURE = ResourceLocation.fromNamespaceAndPath(GreekFantasy.MODID, "textures/entity/satyr/grover_overlay.png");
    ...
    public void render(PoseStack poseStack, MultiBufferSource multiBufferSource, int packedLight, T entity, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
        if (!entity.isInvisible() && entity.hasCustomName() && "Grover".equals(entity.getCustomName().getString())) {
            // In 1.21, coloredCutoutModelCopyLayerRender takes int color instead of 3 floats. -1 = white (0xFFFFFFFF)
            coloredCutoutModelCopyLayerRender(this.getParentModel(), this.layerModel, GROVER_OVERLAY_TEXTURE, poseStack, multiBufferSource, packedLight, entity, limbSwing, limbSwingAmount, ageInTicks, netHeadYaw, headPitch, partialTicks, -1);
        }
    }
}
