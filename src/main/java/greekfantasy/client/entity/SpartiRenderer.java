...
public class SpartiRenderer<T extends Sparti> extends HumanoidMobRenderer<T, SkeletonModel<T>> {
    private static final ResourceLocation TEXTURE = ResourceLocation.fromNamespaceAndPath(GreekFantasy.MODID, "textures/entity/sparti/sparti.png");
    ...
    public SpartiRenderer(EntityRendererProvider.Context context, ModelLayerLocation model, ModelLayerLocation innerArmor, ModelLayerLocation outerArmor) {
        ...
        this.addLayer(new HumanoidArmorLayer<>(this, new SkeletonModel<>(context.bakeLayer(innerArmor)), new SkeletonModel<>(context.bakeLayer(outerArmor)), Minecraft.getInstance().getModelManager()));
