...
public class HydraHeadModel<T extends HydraHead> extends EntityModel<T> {
    public static final ModelLayerLocation HYDRA_HEAD_MODEL_RESOURCE = new ModelLayerLocation(ResourceLocation.fromNamespaceAndPath(GreekFantasy.MODID, "hydra_head"), "hydra_head");
    ...
    public void renderToBuffer(PoseStack poseStack, VertexConsumer vertexConsumer, int packedLightIn, int packedOverlayIn, int color) {
        ...
        severedNeck.render(poseStack, vertexConsumer, packedLightIn, packedOverlayIn, color);
        ...
        if (!severed && !charred && spawnPercent > 0.0F) {
            ...
            neck.render(poseStack, vertexConsumer, packedLightIn, packedOverlayIn, color);
        }
    }
