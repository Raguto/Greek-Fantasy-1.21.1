...
public class FakePigRenderer<T extends LivingEntity> extends LivingEntityRenderer<T, PigModel<T>> {
    ...
    public void render(T entity, float renderOffsetX, float partialTick, PoseStack poseStack, MultiBufferSource multiBufferSource, int packedLight) {
        ...
        // In 1.21, setupRotations takes an extra scale parameter
        this.setupRotations(entity, poseStack, f7, f, partialTick, 1.0F);
        ...
        if (rendertype != null) {
            ...
            // In 1.21, renderToBuffer takes int color instead of 4 floats. ARGB format.
            int color = flag1 ? 0x26FFFFFF : 0xFFFFFFFF; // 0.15 alpha or full white
            this.model.renderToBuffer(poseStack, vertexconsumer, packedLight, i, color);
        }
