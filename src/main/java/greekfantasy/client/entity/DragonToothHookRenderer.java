...
// Removed Matrix3f and Matrix4f imports - now using PoseStack.Pose directly

public class DragonToothHookRenderer extends EntityRenderer<DragonToothHook> {
    private static final ResourceLocation TEXTURE_LOCATION = ResourceLocation.fromNamespaceAndPath(GreekFantasy.MODID, "textures/entity/dragon_tooth_hook.png");
    ...
    // Updated for 1.21 API: uses PoseStack.Pose and method chaining with new method names
    private static void vertex(VertexConsumer vertexConsumer, PoseStack.Pose pose, int packedLight, float x, int y, int u, int v) {
        vertexConsumer.addVertex(pose, x - 0.5F, (float)y - 0.5F, 0.0F)
            .setColor(-1)
    ...
    // Copied from FishingHookRenderer since it's private in 1.21
    ...
    // Updated for 1.21 API: uses method chaining with new method names
    public static void stringVertex(float x, float y, float z, VertexConsumer vertexConsumer, PoseStack.Pose pose, float startPercent, float endPercent) {
        ...
        f4 /= f6;
        f5 /= f6;
        // Color 0xFFFFCC00 = golden/yellow for dragon tooth hook line (ARGB format for setColor(int))
        // Using -52480 which is 0xFFFF3300 in signed int, but let's use the proper method
        vertexConsumer.addVertex(pose, f, f1, f2).setColor(255, 204, 0, 255).setNormal(pose, f3, f4, f5);
    }
