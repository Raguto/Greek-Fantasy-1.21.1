...
public final class GFClientEvents {
    ...
    public static final class ForgeHandler {
        ...
        public static void onRenderLiving(final RenderLivingEvent.Pre<?, ?> event) {
            ...
            if (GreekFantasy.CONFIG.isCurseOfCirceEnabled() && event.getEntity().hasEffect(GFRegistry.MobEffectReg.CURSE_OF_CIRCE)) {
        ...
        public static void onRenderHand(final RenderHandEvent event) {
            ...
            if (GreekFantasy.CONFIG.isCurseOfCirceEnabled()
                    && mc.player.hasEffect(GFRegistry.MobEffectReg.CURSE_OF_CIRCE)) {
                event.setCanceled(true);
                return;
            }
        }
         ...
         */
        // TODO 1.21: TickEvent API changed - need to use new event system
        /*@SubscribeEvent
        public static void onClientTick(final TickEvent.ClientTickEvent event) {
            if (event.phase == TickEvent.Phase.END) {
                final Minecraft mc = Minecraft.getInstance();
                if (mc.player != null && mc.player.jumpableVehicle()!=null && mc.player.getVehicle() instanceof Pegasus pegasus) {
        ...
        public static void onPlayerTick(final TickEvent.PlayerTickEvent event) {
            if (event.phase == TickEvent.Phase.START && event.side == LogicalSide.CLIENT
                ...
                // TODO_ENCHANT: was checking GFRegistry.EnchantmentReg.OVERSTEP
                final boolean hasOverstep = false;
    ...
    @EventBusSubscriber(modid = GreekFantasy.MODID, bus = EventBusSubscriber.Bus.MOD, value = Dist.CLIENT)
    public static final class ModHandler {
        ...
        public static void registerPlayerLayers(final EntityRenderersEvent.AddLayers event) {
            // In 1.21, getSkin() takes a PlayerSkin.Model enum instead of a string
            // Iterate through all skin models and add layers
        ...
        public static void registerItemColors(final RegisterColorHandlersEvent.Item event) {
            event.register((ItemStack item, int tintIndex) -> 0xD8E3D0,
                    BuiltInRegistries.ITEM.get(ResourceLocation.fromNamespaceAndPath(GreekFantasy.MODID, "olive_leaves")));
            event.register((ItemStack item, int tintIndex) -> 0x80f66b,
                    BuiltInRegistries.ITEM.get(ResourceLocation.fromNamespaceAndPath(GreekFantasy.MODID, "golden_leaves")));
        }
        ...
        public static void onRegisterClientExtensions(RegisterClientExtensionsEvent event) {
            // Register spear item extensions (for custom rendering)
            // Note: Spear rendering uses a shared renderer - individual item IDs not needed in event-based registration
            IClientItemExtensions spearExtensions = new IClientItemExtensions() {
                @Override
                public net.minecraft.client.renderer.BlockEntityWithoutLevelRenderer getCustomRenderer() {
                    return greekfantasy.client.blockentity.BlockEntityRendererProvider.getSpear(ResourceLocation.fromNamespaceAndPath(GreekFantasy.MODID, "spear"));
                }
            };
            event.registerItem(spearExtensions,
                    GFRegistry.ItemReg.BIDENT.get(),
                    GFRegistry.ItemReg.WOODEN_SPEAR.get(),
                    GFRegistry.ItemReg.FLINT_SPEAR.get(),
                    GFRegistry.ItemReg.STONE_SPEAR.get(),
                    GFRegistry.ItemReg.GOLDEN_SPEAR.get(),
                    GFRegistry.ItemReg.IRON_SPEAR.get(),
                    GFRegistry.ItemReg.DIAMOND_SPEAR.get(),
                    GFRegistry.ItemReg.NETHERITE_SPEAR.get());

            // Register head item extensions (for custom block entity rendering)
            event.registerItem(new IClientItemExtensions() {
                @Override
                public net.minecraft.client.renderer.BlockEntityWithoutLevelRenderer getCustomRenderer() {
                    return greekfantasy.client.blockentity.BlockEntityRendererProvider.getCerberusHead();
                }
            }, GFRegistry.ItemReg.CERBERUS_HEAD.get());
            ...
            }, GFRegistry.ItemReg.GIGANTE_HEAD.get());
            ...
            }, GFRegistry.ItemReg.ORTHUS_HEAD.get());

            // Register armor item extensions (for custom armor models)
            event.registerItem(new IClientItemExtensions() {
                ...
                public net.minecraft.client.model.HumanoidModel<?> getHumanoidArmorModel(LivingEntity entityLiving, ItemStack itemStack, EquipmentSlot armorSlot, net.minecraft.client.model.HumanoidModel<?> _default) {
                    ...
                    GFRegistry.ItemReg.HELLENIC_HELMET.get(),
                    GFRegistry.ItemReg.HELLENIC_CHESTPLATE.get(),
                    GFRegistry.ItemReg.HELLENIC_LEGGINGS.get(),
                    GFRegistry.ItemReg.HELLENIC_BOOTS.get());
            ...
            }, GFRegistry.ItemReg.NEMEAN_LION_HIDE.get());
            ...
            }, GFRegistry.ItemReg.WINGED_SANDALS.get());
        }
        ...
        private static void registerUsingProperties(final Item usingItem, final String propertyName) {
            ItemProperties.register(usingItem, ResourceLocation.withDefaultNamespace(propertyName),
        ...
        private static void registerBowProperties(final Item bow) {
            ItemProperties.register(bow, ResourceLocation.withDefaultNamespace("pull"),
                    (item, world, entity, tintIndex) -> {
                        ...
                        return (item.getUseDuration(entity) - entity.getUseItemRemainingTicks()) / 20.0F;
                    });
        ...
        private static void registerFishingRodProperties(final Item rod) {
            ItemProperties.register(rod, ResourceLocation.withDefaultNamespace("cast"), (item, level, entity, tintIndex) -> {
                if (entity == null) {
                    return 0.0F;
                } else {
        ...
        private static void registerSpearProperties(final Item spear) {
            ItemProperties.register(spear, ResourceLocation.withDefaultNamespace("throwing"), (item, world, entity, tintIndex) ->
