...
public class SSongPacket {
    ...
    public static SSongPacket fromBytes(final FriendlyByteBuf buf) {
        final Map<ResourceLocation, Song> data = buf.readWithCodec(NbtOps.INSTANCE, CODEC, NbtAccounter.unlimitedHeap());
     ...
     */
    public static void handlePacket(final SSongPacket message, final IPayloadContext context) {
        context.enqueueWork(() -> {
            GreekFantasy.SONG_MAP.clear();
